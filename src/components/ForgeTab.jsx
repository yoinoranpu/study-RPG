...
import { RARITY_COLOR, RARITY_LABEL, RARITY, getSellPrice, getItemStats, INNATE, SYNTHESIS_COST, getAbilitySlots, getAbilityTier, rollAbility } from "../data/items";
...
export default function ForgeTab() {
  ...
  const [matSel, setMatSel] = useState(null); // 合成の素材武器
  ...
  // 同じsubtype・同じrarity・別のuidのアイテム
  const synthCandidates = baseItem
    ? itemBox.filter(it =>
        it.uid !== baseItem.uid &&
        it.type === baseItem.type &&
        it.rarity === baseItem.rarity
      )
    : [];
  ...
  function synthesize() {
    ...
    // ランダム能力も強化
    const newAbilities = (baseItem.abilities||[]).map(ab => ({
      ...ab,
      value: Math.ceil(ab.value * 1.3),
    }));
    ...
    if (matAbility) {
      ...
      if (existIdx >= 0) {
        if (matAbility.value > newAbilities[existIdx].value) {
          newAbilities[existIdx] = { ...matAbility, value: Math.ceil(matAbility.value * 1.3) };
        }
    ...
    updatePlayer({
      itemBox: itemBox
        .filter(x => x.uid !== matItem.uid)
        .map(x => x.uid === baseItem.uid ? synthesized : x),
      ...
      <div style={{ flex:1, overflowY:"auto", padding:10 }}>
        ...
        {tab === "upgrade" && (
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
              ...
              {itemBox.filter(it=>["weapon","armor","accessory"].includes(it.type)).map(it => {
                ...
                return (
                  <div key={it.uid} onClick={() => setSel(isSel?null:it.uid)}
                    ...
                    <div style={{ fontSize:4, color:tc, textAlign:"center", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap", width:"100%", paddingLeft:1, paddingRight:1 }}>{it.name.slice(0,4)}</div>
                  </div>
                );
              })}
            </div>
            ...
            {upgradeItem && (
              <div style={{ background:"#0d0d15", border:`1px solid ${RARITY_COLOR[upgradeItem.type]||"#2a2a3a"}`, borderRadius:8, padding:12 }}>
...
{/* 現在のステータス */}
        ...
        {tab === "synth" && (
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            <div style={{ fontSize:8, color:"#4a4a6a", padding:"4px 0" }}>
              同じ種類・同じレアリティの装備2つで1段階上のレアリティに合成
            </div>
                ...
                {itemBox.filter(it=>["weapon","armor","accessory"].includes(it.type) && nextRarity(it.rarity)).map(it => {
                  ...
                  return (
                    <div key={it.uid} onClick={() => { setSel(isSel?null:it.uid); setMatSel(null); }}
                      style={{ aspectRatio:"1", background:isSel?"#12122a":"#080810", border:`2px solid ${isSel?rc:rc+"33"}`, borderRadius:5, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", cursor:"pointer", padding:1, position:"relative" }}>
                      <div style={{ fontSize:13 }}>{it.icon}</div>
            ...
            {baseItem && (
              <div>
                <div style={{ fontSize:8, color:"#a78bfa", marginBottom:6 }}>
                  ② 素材装備を選択（{baseItem.name}と同レアリティ・同種類）
                </div>
...
{/* 選択中アイテムの詳細 */}
            ...
            {baseItem && matItem && next && (
              <div style={{ background:"#0d0d15", border:`1px solid ${RARITY_COLOR[next]||"#2a2a3a"}`, borderRadius:8, padding:12 }}>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:8 }}>
                  <span style={{ fontSize:9, color:RARITY_COLOR[baseItem.rarity] }}>{baseItem.name}</span>
                  <span style={{ color:"#4a4a6a" }}>+</span>
                  <span style={{ fontSize:9, color:RARITY_COLOR[matItem.rarity] }}>{matItem.name}</span>
                  <span style={{ color:"#4a4a6a" }}>→</span>
                  <span style={{ fontSize:9, color:RARITY_COLOR[next], fontWeight:700 }}>
                    {baseItem.name} {RARITY_LABEL[next]}
                  </span>
                </div>
                ...
                {(matItem.abilities||[]).length > 0 && (
                  <div style={{ fontSize:8, color:"#a78bfa", marginBottom:8 }}>
                    継承される能力: {matItem.abilities[0]?.label}{matItem.abilities[0]?.value}{matItem.abilities[0]?.suffix}
                    <span style={{ color:"#4a4a6a", marginLeft:4 }}>（素材の1番目の能力）</span>
                  </div>
                )}
