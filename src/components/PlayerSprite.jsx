...
// 歩行アニメーション（プレースホルダー・ドット絵風）
function drawPlayer(ctx, x, y, frame, hp, maxHp) {
  const scale = 2;
  const W = 24 * scale;
  const H = 32 * scale;
  const bx = x - W / 2;
...
export default function PlayerSprite({ hp, maxHp, isRunning, isBreak }) {
  ...
  useEffect(() => {
    ...
    const loop = (time) => {
      ...
      drawPlayer(ctx, px, py, animFrame.current, hp, maxHp);
