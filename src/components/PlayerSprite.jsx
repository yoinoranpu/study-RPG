...
function drawPlayerHpBar(ctx, x, by, W, hp, maxHp) {
  ...
  ctx.beginPath();
  ctx.roundRect(barX, barY, barW * hpPct, barH, 5);
  ctx.fill();
  ctx.restore();
  ctx.fillStyle = "rgba(255,255,255,0.18)";
  ctx.beginPath();
  ctx.roundRect(barX + 2, barY + 2, Math.max(0, barW * hpPct - 4), barH * 0.32, 3);
  ctx.fill();
...
export default function PlayerSprite({ hp, maxHp, isRunning, isBreak, hitTrigger, isDefeated }) {
  const canvasRef    = useRef(null);
  const frameRef     = useRef(null);
  const animFrame    = useRef(0);
  const hitFlashRef  = useRef(0);
  const defeatFrame  = useRef(0);
  ...
  useEffect(() => {
    ...
    const loop = (time) => {
      ...
      if (isRunning && !isBreak) animFrame.current += dt * 0.05;
      hitFlashRef.current = Math.max(0, hitFlashRef.current - dt);

      const bobY = isRunning && !isBreak ? Math.sin(animFrame.current * 0.3) * 3 : 0;
      const px = W * 0.72;
      const py = H * 0.72 + bobY;

      if (isDefeated) {
        defeatFrame.current += dt * 0.0005;
  ...
  }, [isRunning, isBreak, hp, maxHp, isDefeated]);
