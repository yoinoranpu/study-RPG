...
export default function SkillBookTab() {
  ...
  const Row = ({ book }) => {
    ...
    const rc = discovered ? BOOK_RARITY_COLOR[entry.bestRarity] || "#888" : "#888";
    return (
      <div style={{ background:"#0d0d15", border:`1px solid ${discovered?rc+"44":"#1a1a2a"}`, borderRadius:6, padding:"10px 12px", marginBottom:6, opacity:discovered?1:0.5 }}>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          <div style={{ width:36, height:36, background:discovered?rc+"22":"#1a1a2a", borderRadius:6, display:"flex", alignItems:"center", justifyContent:"center", fontSize:18, filter:discovered?"none":"grayscale(1) brightness(0.3)" }}>
          ...
          <div style={{ flex:1 }}>
            ...
            <div style={{ fontSize:8, color:discovered?"#4a4a6a":"#2a2a2a" }}>
              {discovered ? book.desc : "未発見"}
            </div>
          </div>
          {discovered && (
            <div style={{ textAlign:"right" }}>
              <div style={{ fontSize:9, color:rc }}>{BOOK_RARITY_LABEL[entry.bestRarity]}</div>
              <div style={{ fontSize:7, color:"#4a4a6a" }}>累計×{entry.count}</div>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", height:"100%", fontFamily:"monospace" }}>
      <div style={{ padding:"8px 12px", background:"#080810", borderBottom:"1px solid #1a1a2a", flexShrink:0 }}>
        <div style={{ fontSize:8, color:"#a78bfa", letterSpacing:2 }}>SKILL BOOK DEX</div>
      ...
      <div style={{ flex:1, overflowY:"auto", padding:10 }}>
        <div style={{ fontSize:7, color:"#f87171", letterSpacing:2, marginBottom:6 }}>アクティブ書</div>
        {active.map(b => <Row key={b.id} book={b} />)}
        <div style={{ fontSize:7, color:"#a78bfa", letterSpacing:2, margin:"10px 0 6px" }}>パッシブ書</div>
