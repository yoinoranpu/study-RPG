      ...
      <div style={{ display:"flex", alignItems:"center", gap:8, padding:"6px 8px", background:equipped?`${color}18`:"#080810", border:`1px solid ${equipped?color:rc+"44"}`, borderRadius:4, marginBottom:4 }}>
        ...
        <div style={{ flex:1 }}>
          <div style={{ fontSize:10, color:"#e8e0d0" }}>
            {book.name} <span style={{ fontSize:7, color:rc }}>{BOOK_RARITY_LABEL[owned.rarity]}</span>
          </div>
          <div style={{ fontSize:8, color:"#4a4a6a" }}>{book.desc}</div>
        </div>
      ...
      {tab === "equip" && (
        <>
            ...
            <div style={{ padding:"10px 10px", display:"flex", flexDirection:"column", gap:8, minWidth:110 }}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:4 }}>
                {EQUIP_SLOTS.map(({key,label,icon})=>{
                  ...
                  return (
                    <div key={key} onClick={() => eq && setSel(sel===`slot_${key}`?null:`slot_${key}`)} title={label}
              ...
              <div style={{ display:"flex", gap:4 }}>
                {(player.specialSlots||[null,null,null]).map((slot,i)=>(
                  <div key={i} onClick={() => slot && setSel(sel===`cslot_${i}`?null:`cslot_${i}`)}
          ...
          <div style={{ flex:1, overflowY:"auto", padding:"8px 10px" }}>
            <div style={{ fontSize:7, color:"#4a4a6a", letterSpacing:2, marginBottom:6 }}>ITEM BOX {(itemBox||[]).length}/30</div>
              ...
              {(itemBox||[]).map(it=>{
                ...
                return (
                  <div key={it.uid} onClick={()=>setSel(isSel?null:it.uid)}
                    ...
                    {it.upgradeLevel>0 && <div style={{ position:"absolute", top:1, right:3, fontSize:7, color:"#fbbf24", fontWeight:700 }}>+{it.upgradeLevel}</div>}
                    {isEq && <div style={{ position:"absolute", bottom:0, left:0, right:0, fontSize:5, color:"#4ade80", textAlign:"center", background:"rgba(0,0,0,0.7)", borderRadius:"0 0 4px 4px" }}>装備中</div>}
        ...
        <div style={{ flex:1, overflowY:"auto", padding:12 }}>
          ...
          <div style={{ background:"#0d0d15", border:"1px solid #2a2a3a", borderRadius:8, padding:12, marginBottom:10 }}>
            <div style={{ fontSize:8, color:"#f87171", letterSpacing:2, marginBottom:8, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
              <span>アクティブ書（最大4）</span>
              <div style={{ display:"flex", gap:4 }}>
                <button onClick={() => updatePlayer({ skillMode:"order" })}
                  style={{ padding:"3px 8px", background:(skillMode||"order")==="order"?"#1a0a0a":"transparent", border:`1px solid ${(skillMode||"order")==="order"?"#f87171":"#2a2a3a"}`, borderRadius:3, cursor:"pointer", color:(skillMode||"order")==="order"?"#f87171":"#4a4a6a", fontSize:8, fontFamily:"monospace" }}>🔢 順番</button>
                <button onClick={() => updatePlayer({ skillMode:"random" })}
                  style={{ padding:"3px 8px", background:skillMode==="random"?"#1a0a0a":"transparent", border:`1px solid ${skillMode==="random"?"#f87171":"#2a2a3a"}`, borderRadius:3, cursor:"pointer", color:skillMode==="random"?"#f87171":"#4a4a6a", fontSize:8, fontFamily:"monospace" }}>🎲 ランダム</button>
              </div>
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:6, marginBottom:8 }}>
              {actSlots.map((uid,i)=>{
                ...
                const rc = owned ? BOOK_RARITY_COLOR[owned.rarity] : "#2a2a3a";
                return (
                  <div key={i} style={{ background:"#080810", border:`1px solid ${book?rc:"#2a2a3a"}`, borderRadius:5, padding:"6px 8px", display:"flex", alignItems:"center", gap:6, minHeight:36 }}>
                    <span style={{ fontSize:9, color:"#3a3a5a" }}>S{i+1}</span>
                    {book ? (<>
                      <span style={{ fontSize:12 }}>{book.icon}</span>
                      <div style={{ flex:1 }}><div style={{ fontSize:9, color:rc }}>{book.name}</div></div>
            ...
            {ownedActive.length === 0
              ? <div style={{ fontSize:9, color:"#2a2a2a", textAlign:"center", padding:8 }}>アクティブ書を持っていない</div>
          ...
          <div style={{ background:"#0d0d15", border:"1px solid #2a2a3a", borderRadius:8, padding:12 }}>
            <div style={{ fontSize:8, color:"#a78bfa", letterSpacing:2, marginBottom:8 }}>パッシブ書（最大6）</div>
            <div style={{ display:"flex", flexWrap:"wrap", gap:4, marginBottom:10 }}>
              {pasSlots.map((uid,i)=>{
                ...
                const rc = owned ? BOOK_RARITY_COLOR[owned.rarity] : "#2a2a3a";
                return (
                  <div key={i} style={{ background:"#080810", border:`1px solid ${book?rc:"#2a2a3a"}`, borderRadius:4, padding:"4px 8px", fontSize:8, display:"flex", alignItems:"center", gap:4 }}>
            ...
            {ownedPassive.length === 0
              ? <div style={{ fontSize:9, color:"#2a2a2a", textAlign:"center", padding:8 }}>パッシブ書を持っていない</div>
