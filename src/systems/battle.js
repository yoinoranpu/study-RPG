...
export const simulateBattle = (player, monsters, options = {}) => {
  ...
  // ─── 実効ステータス（ランク補正込み）───
  const eAtk  = (e) => Math.floor((e.atk  + (e.side==="player" ? atkScale : 0)) * rankMul(e.ranks?.atk));
  const eMag  = (e) => Math.floor(e.mag  * rankMul(e.ranks?.mag));
  ...
  const eEva  = (e) => Math.floor(e.eva  * rankMul(e.ranks?.eva));
  ...
  // ─── 召喚物の行動 ───
  const allyAct = (ally) => {
    const skipReason = checkSkipTurn(ally);
    if (skipReason) return;
    const target = pickEnemyTarget();
    if (!target) return;
    const baseStat = ally.statRef === "mag" ? eMag(playerEntity) : eAtk(playerEntity);
    let dmg = calcDamage(Math.floor(baseStat * ally.atkMul), eDef(target));
    if (Math.random()*100 < eEva(target)) {
  ...
  const enemyAct = (enemy) => {
    ...
    // 敵の状態異常付与（モンスターに status プロパティがあれば）
    if (action.status) {
      tryApplyStatus(playerEntity, action.status, eMag(enemy));
    }
  ...
  const processStatusTick = (entity) => {
    ...
    results.forEach(r => {
      ...
      if (entity.side === "enemy") {
        ...
        // 毒蓄積スキル
        if (r.type === "poison" && learned.has("th_toxicscale")) atkScale += 3;
        if (entity.hp <= 0) {
