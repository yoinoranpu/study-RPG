...
export const simulateBattle = (player, monsters) => {
  ...
  const doPlayerAttack = () => {
    monsters.forEach((mon, i) => {
      ...
      if (player.vampiric) playerHp = Math.min(player.maxHp, playerHp + Math.floor(dmg * player.vampiric / 100));
  ...
  const doEnemyAttack = () => {
    monsters.forEach((mon, i) => {
      ...
      if (action.type === "skip") { addLog(`  ${mon.displayName}:${action.label}`, "#3a3a3a"); turns.push({ actor:"monster", source:i, type:"skip" }); return; }
      if (action.type === "def")  { addLog(`  ${mon.displayName}:${action.label}`, "#60a5fa"); turns.push({ actor:"monster", source:i, type:"defend" }); return; }
      if (Math.random() * 100 < player.eva) { addLog("  ✨ 回避！", "#34d399"); turns.push({ actor:"monster", source:i, type:"miss" }); return; }
      ...
      addLog(`  ${mon.displayName}の${action.label}！${dmg}ダメ(自HP${playerHp})`, action.type === "matk" ? "#a78bfa" : "#f87171");
